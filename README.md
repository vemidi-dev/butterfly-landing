...
- Куриер: Еконт / Спиди
- Доставка: до офис (`office`) / до автомат (`locker`) / до адрес (`address`)
- Град и офис/адрес (ръчно въвеждане)
- Плащане: **само наложен платеж**
- Потвърждение за безопасност (задължително)
...
### Ръчно въвеждане при липсваща courier интеграция

Засега може да няма активни Speedy/Еконт API credentials или courier API да не върне желания
офис. За да не блокираме поръчката:

- при **доставка до офис** клиентът може да избере офис от списъка **или** да напише желания
  офис ръчно в полето „Не намираш желания офис?“. Поръчката минава, ако има избран офис
  **или** попълнен ръчен текст;
- при **доставка до автомат** (`locker`) courier API **не** се ползва — показва се само град и
  поле за ръчно въвеждане на автомата;
- ръчно въведеният офис/автомат се пази в `delivery.manualOfficeNote` (в `raw_payload`) и при
  липса на избран офис се записва и в `delivery_details`. Не се добавя нова колона в базата.

> Товарителници не се създават автоматично. Екипът се свързва с клиента, за да уточни офис/автомат
> и детайлите по доставката.
...
Имейлът съдържа: клиент, телефон, имейл, комплект, оцветяване, персонализация, име, тип доставка (офис/автомат/адрес), куриер, град, избран офис, ръчно въведен желан офис/автомат, обща цена, бележка, потвърждение за безопасност.
