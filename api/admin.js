...
function handleLogin(req, res) {
  ...
  if (!password) return json(res, 400, { ok: false, error: 'Въведи парола.' });
  ...
  if (req.method === 'GET') {
    try {
      ...
      return json(res, 200, {
        ok: true,
        orders,
        statuses: ORDER_STATUSES.map((value) => ({ value, label: STATUS_LABELS[value] })),
      });
    } catch (err) {
...
module.exports = async function handler(req, res) {
  ...
  if (!action) return json(res, 400, { ok: false, error: 'Липсва action параметър.' });

  if (action === 'login') return handleLogin(req, res);
  if (action === 'session') return handleSession(req, res);
  if (action === 'orders') return handleOrders(req, res);

  return json(res, 400, { ok: false, error: 'Невалиден action параметър.' });
};
