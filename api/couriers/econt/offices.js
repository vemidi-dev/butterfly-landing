...
module.exports = async function handler(req, res) {
  ...
  try {
    const offices = await fetchOffices('econt', city);
    return json(res, 200, { ok: true, offices });
  } catch (err) {
    if (err.code === 'MISSING_COURIER_ENV') {
      return json(res, 500, { ok: false, error: 'Липсва конфигурация за Еконт API.' });
    }
    console.error('[couriers/econt/offices]', {
      status: err.status || null,
      message: err.message,
    });
    return json(res, 502, { ok: false, error: 'Неуспешно зареждане на офиси от Еконт.' });
  }
};
