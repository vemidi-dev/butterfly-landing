...
/** Reverse map: store option value key → landing size (3/5/7). */
const STORE_TO_LANDING_SIZE_BY_VALUE = {
...
const STORE_TO_LANDING_VALID_SIZES = new Set(['3', '5', '7']);
const STORE_TO_LANDING_VALID_COLORING = new Set(['paints', 'markers']);
...
/**
 * Reverse-map store cross-handoff fields to landing form state (browser-safe).
 * @param {Record<string, string>} fields
 * @returns {{ size: string, coloring: string, personalize: boolean, personalizationName: string } | null}
 */
function mapStoreHandoffFieldsToFormState(fields = {}) {
  ...
  const size = STORE_TO_LANDING_SIZE_BY_VALUE[sizeKey];
  if (!size || !STORE_TO_LANDING_VALID_COLORING.has(coloring)) {
    return null;
  }

  const rawName = fields[LEGACY_PERSONALIZATION_POST_KEY];
...
/**
 * @param {{ size?: string, coloring?: string, personalize?: boolean, personalizationName?: string }} formState
 * @returns {boolean}
 */
function isValidStoreHandoffFormState(formState) {
  if (!formState || !STORE_TO_LANDING_VALID_SIZES.has(String(formState.size || ''))) {
    return false;
  }
  if (!STORE_TO_LANDING_VALID_COLORING.has(String(formState.coloring || ''))) {
    return false;
  }
...
const api = {
  ...
  STORE_TO_LANDING_SIZE_BY_VALUE,
  STORE_TO_LANDING_VALID_SIZES,
  STORE_TO_LANDING_VALID_COLORING,
  mapStoreHandoffFieldsToFormState,
  isValidStoreHandoffFormState,
